        ...
        <table className='w-1/2 mx-auto my-4 divide-y divide-green-500'>
          ...
          <tbody>
            {data.map((stand, dIndex) => {
              return (
                <tr key={dIndex}>
                  <td key={dIndex} className='flex justify-between p-4'>
                  ...
                  {hours.map((_, index) => {
                    ...
                    return <td key={index}>{salesPerHour}</td>
                  })}
                  <th key={dIndex + 1}>
                    {hourly_sales.reduce(
                      (partial_sum, a) => partial_sum + a,
          ...
          <tfoot>
            <tr>
              ...
              <th>{totals.reduce((partial_sum, a) => partial_sum + a, 0)}</th>
            </tr>
          </tfoot>
        </table>
      ) : (
